...
object Conexion {
    ...
    fun buscarPersona(contexto: AppCompatActivity, dni:String):Persona? {
        ...
        val fila =bd.rawQuery(
            "SELECT nombre, apellido,equipo FROM personas WHERE dni=?",
        ...
        if (fila.moveToFirst()) {//si no hay datos el moveToFirst, devuelve false, si hay devuelve true.
            p = Persona(dni, fila.getString(0), fila.getString(1),fila.getString(2))
        }
        bd.close()
        return p
    }
    ...
    fun obtenerPersonas(contexto: AppCompatActivity):ArrayList<Persona>{
        ...
        val admin = AdminSQLIteConexion(contexto, this.DATABASE_NAME, null, DATABASE_VERSION)
        val bd = admin.readableDatabase
        val fila = bd.rawQuery("select dni,nombre,apellido,equipo from personas", null)
        while (fila.moveToNext()) {
            var p:Persona = Persona(fila.getString(0),fila.getString(1),fila.getString(2),fila.getString(3))
            personas.add(p)
        }
        bd.close()
